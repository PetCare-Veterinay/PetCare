...
@section('content')
    ...
    <div class="content" style="background-color: #fff; border: 2px solid #fff; padding: 35px;">
        ...
        <div class="d-flex justify-content-between mb-3">
            ...
            <a href="{{ route('DiNuevo') }}" id="load-create-view" class="btn btn-primary large_button"> + Agregar</a>
        </div>
        <!-- Lista de Datos -->
        <div class="box">
            <div class="box-body">
                <table class="table table-hover custom-table">
                    <thead>
                        <tr>
                            ...
                            <th scope="col">Examenes_Laboratorio</th>
                            <th scope="col">Recomendaciones</th>
                            <th scope="col">Plan_Seguimiento</th>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>¿Estás seguro de que deseas eliminar este diagnostico?</p>
                </div>
...
fetch('/api/diagnostico')
        ...
        .then(data => {
            ...
            data.forEach(user => {
                ...
                const VacunasCell = document.createElement('td');
                VacunasCell.textContent = user.Vacunas;

                const Examenes_LaboratorioCell = document.createElement('td');
                Examenes_LaboratorioCell.textContent = user.Examenes_Laboratorio;

                const RecomendacionesCell = document.createElement('td');
                RecomendacionesCell.textContent = user.Recomendaciones;
                
                const Plan_SeguimientoCell = document.createElement('td');
                Plan_SeguimientoCell.textContent = user.Plan_Seguimiento;

                const EnfermedadesCell = document.createElement('td');
                EnfermedadesCell.textContent = user.Enfermedades;
                ...
                const id = user.id;
                window.location.href = `/1diagnostico/${user.id}`; 
                });
                ...
                deleteButton.setAttribute('data-delete', `api/diagnostico/${user.id}`);
...
row.appendChild(VacunasCell);
row.appendChild(Examenes_LaboratorioCell);
row.appendChild(RecomendacionesCell);
row.appendChild(Plan_SeguimientoCell);
row.appendChild(EnfermedadesCell);
...
<script>
        $(document).ready(function() {
            $("#load-create-view").click(function() {
                ...
                $.get("{{ route('DiNuevo') }}", function(data) {
